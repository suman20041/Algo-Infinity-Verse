...
function renderProblemCardHtml(problem) {
  const userProgress = window.userProgress || {};
  const cpType = userProgress.codingPersonality
    ? userProgress.codingPersonality.type
    : 'brute-force first';
  ...
  if (cpType === 'brute-force first') {
    ...
    if (
      problem.tags.includes('Greedy') ||
      problem.tags.includes('Divide and Conquer') ||
      problem.tags.includes('Recursion')
    ) {
      isRec = true;
  ...
  const snippetHtml = problem.highlightedDescription
    ? `<div class="problem-snippet" style="font-size: 0.85em; color: var(--text-secondary); margin-bottom: 8px; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">${problem.highlightedDescription}</div>`
    : '';
...
function createSparkles(button) {
  ...
  const numSparkles = 8;
  const colors = ['#ff4d6d', '#ff758f', '#ff8fa3', '#ffb3c1', '#ffd6ff'];

  for (let i = 0; i < numSparkles; i++) {
    const sparkle = document.createElement('div');
    sparkle.className = 'fav-sparkle';

    const size = Math.random() * 6 + 4; // 4px to 10px
    const color = colors[Math.floor(Math.random() * colors.length)];
    const angle = (i * (360 / numSparkles) + Math.random() * 15) * (Math.PI / 180);
    const distance = Math.random() * 25 + 25; // 25px to 50px
...
function handleFavoriteClick(favoriteBtn, problemId) {
  const userProgress = window.userProgress || {};
  const isCurrentlyFavorite = userProgress.favoriteProblems.includes(problemId);

  toggleFavorite(problemId);

  if (currentFilter === 'favorites') {
    const card = favoriteBtn.closest('.problem-card');
    if (card) {
...
function attachProblemGridEventDelegation(grid) {
  ...
  grid.addEventListener('click', (e) => {
    ...
    if (favoriteBtn && grid.contains(favoriteBtn)) {
      ...
      handleFavoriteClick(favoriteBtn, problemId);
      return;
    }
...
function toggleFavorite(problemId) {
  ...
  if (idx > -1) {
    ...
    removeProblemFromCollections(
      userProgress,
      problemId,
      getCollectionsForProblem(userProgress, problemId)
    );
...
function handleProblemClick(problemId) {
  ...
  if (problem) {
    if (typeof openQuizEditor === 'function') openQuizEditor(problem);
